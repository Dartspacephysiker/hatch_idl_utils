...
;	97-5-14		J.McFadden
;-
function j_2d_b__real2d,dat2,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins
...
if ind ge 0 then begin
	...
	;; domega=2.*dphi*sin(theta)*sin(.5*esa_dth)
	domega=2.*sin(.5*esa_dth)
endelse
...
flux3dx = total(dnrg*data*domega*cos(theta))
;; flux3dy = total(dnrg*sumdatay)
flux3dy = 0.D
flux3dz = total(dnrg*data*domega*sin(theta))
