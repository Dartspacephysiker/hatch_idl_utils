...
FUNCTION DIFF_ENERGY_FLUX_SPECTRAL_TYPE__FAST_ADJ_V2,eSpec,Je,Jee, $
  ...
  IF cont THEN BEGIN
     ...
     IF ~cont THEN BEGIN
        mono       = -2
        mFCode     = mFCode OR 2
        IF KEYWORD_SET(produce_failCodes) THEN cont = 1
     ENDIF
  ENDIF
  ...
  IF cont THEN BEGIN
     mono                     = 1 + (strictMono_candidate_above AND strictMono_candidate_below)
  ENDIF
  ...
  event = { x:time_e, $         ; When are you?
            ...
            version:DEF_ST_VERSION_NUM}                     ;0 = no problems      , N = N bad bins
