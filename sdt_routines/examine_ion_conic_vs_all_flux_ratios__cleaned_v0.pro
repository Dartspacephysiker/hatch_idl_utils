...
PRO EXAMINE_ION_CONIC_VS_ALL_FLUX_RATIOS__CLEANED_V0, $
   TIMES=times, $
  ...
  IF KEYWORD_SET(do_not_enforce_sample_rate) THEN BEGIN
     PRINT,"EXAMINE_ION_CONIC_VS_ALL_FLUX_RATIOS__CLEANED_V0: Not enforcing sample rate ..."
  ...
  GET_DIFF_EFLUX__CLEANED_V0,T1=times[0],T2=times[-1], $
                             EEB_OR_EES=ieb_or_ies, $
                             SC_POT=sc_pot, $
                             ENFORCE_DIFF_EFLUX_SRATE=enforce_diff_eFlux_sRate, $
