;;2018/03/07
;;timeTmp is an array of DOUBLES, each of which is a UTC time expressed as seconds since 01/01/1970 00:00:00 UTC
;;geoStruct is a structure with members lat, lon, and alt
...
FUNCTION CONVERT_GEO_TO_AACGM__SINGLE_SET, $
   ...
   OUTDIR=outDir, $
   ALLOW_FL_TRACE=allow_FL_trace, $
   OUTPUT__TIMEFILENAME=output__timeFileName, $
   OUTPUT__SAVEFILENAME=output__saveFileName, $
   OUTPUT__AACGMSPH=AACGMSph, $
   RESTORE_LASTCONV=restore_lastConv
  ...
  IF KEYWORD_SET(restore_lastConv) THEN BEGIN
     ...
     output__saveFileNameLast = 'last_conv_geo_to_aacgm__single_set__AACGM.sav'

     ;; KEYWORD_SET(output__saveFileName) ? output__saveFileName : 

     ;; PRINT,"outDir = '/SPENCEdata/Research/Satellites/FAST/kappa_dists/saves_output_etc/'"
     ;; PRINT,"output__timeFileName = 'last_conv_geo_to_aacgm__single_set__tStamps.sav'"
     ;; PRINT,"output__saveFileName = '" + output__saveFileName + "'"
  ...
  ;; FOR i=0,N_ELEMENTS(coordFiles)-1 DO BEGIN
     ...
     conv_tStamps = 1
     IF KEYWORD_SET(output__timeFileName) THEN IF ~(CHECK_NEED_TO_RECALC_TSTAMPS( $
        output__timeFileName,timeTmp,nTot, $
        ALTITUDE_MAX=altitude_max, $
        R_E=R_E, $
        ALLOW_FL_TRACE=allow_fl_trace, $
        OUT_TIMETMPSTR=timeTmpStr, $
     ...
     IF conv_tStamps THEN BEGIN

        IF KEYWORD_SET(restore_lastConv) THEN BEGIN
           RESTORE,lastOutDir+output__timeFileNameLast
        ENDIF ELSE BEGIN
           timeTmpStr = CREATE_FASTDB_TSTAMPS(timeTmp,nTot,output__timeFileName, $
                                              ;; ALTITUDE_MAX=altitude_max, $
                                              ;; R_E=R_E, $
                                              ;; ALLOW_FL_TRACE=allow_fl_trace, $
                                              DONT_SAVE_TIMESTAMPS=~(SIZE(output__timeFileName,/TYPE) EQ 7))
           SAVE,timeTmpStr,FILENAME=outDir+output__timeFileNameLast
        ENDELSE
     ENDIF
