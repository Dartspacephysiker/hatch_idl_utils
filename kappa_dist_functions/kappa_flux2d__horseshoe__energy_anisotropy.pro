...
FUNCTION KAPPA_FLUX2D__HORSESHOE__ENERGY_ANISOTROPY,X,Y,P, $
   ...
   MU_0=mu_0, $
   BULK_E_ANISOTROPY=bulk_e_anisotropy, $
  ...
  IF N_ELEMENTS(bulk_e_anisotropy) EQ 0 THEN BEGIN
     bulk_e_anisotropy = MAKE_ARRAY(nAngles,VALUE=1.0)
  ENDIF ELSE BEGIN
     ;; PRINT,'Bulking out'
  ENDELSE
  ...
  FOR i=0,nAngles-1 DO BEGIN
     ...
     tempEn   = X[*,i]
     tempP    = P
     tempP[0] = tempP[0]*bulk_e_anisotropy[i]
