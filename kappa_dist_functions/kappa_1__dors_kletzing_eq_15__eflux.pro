...
FUNCTION KAPPA_1__DORS_KLETZING_EQ_15__EFLUX,kappa,T_m,dens_m,pot,R_B, $
  ...
  Finv                   = n * electron_mass * (w_sq)^(1.5D) / 4.D / SQRT(!PI) * kappa^(2.D) * A_k * R_B / ( ( kappa - 1.D ) * ( kappa - 2.D ) )

  ;;First chunk
  FK1                    = 2.D + (kappa - 2.D) / (kappa - 1.5D) * potBar

  ;;Second chunk
  FK2                    = ( (kappa - 2.D) / (kappa - 1.D) + potBar * (kappa - 2.D) / (kappa - 1.5D) ) * ( kappa / ( (kappa - 1.D) * (R_B - 1.D) ) + 1.D )

  ;;Third chunk, in parts that become useful later for PDs
  FK3                    = 1.D + ( 1.D + kappa / ( R_B - 1.D ) ) / ( kappa - 1.D )
